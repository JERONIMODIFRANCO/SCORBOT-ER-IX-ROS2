...
//###########################################################################
//
// Archivos de inclusi�n
//
#include "F28x_Project.h" //este esta en el lac y no me acuerdo pq no podiamos tenerlo 
#include "DCLF32_scorbot.h" //RUTINAS DE CONTROL
...
// Funciones prototipo
//
interrupt void adca1_isr(void); //adc para medir las varialbes corriente y pos
interrupt void xint_SE_isr(void); //rutina de interrupcion por emergencia
interrupt void xint_OCP_isr(void); // rutina de interrupcion sobrecorriente
interrupt void xint_OTP_isr(void); // rutina de interrupcion sobretemperatura
//
// Variables globales
//
volatile int32 doSample = 0; //se hizo una medicion
volatile int32 doLoop = 0; // lazo de control
volatile int HOME_SW_1 = 0, HOME_SW_2 = 0, HOME_SW_3 = 0; //varialbes home switch 
volatile int SE_IN_ISR=0, OCP_IN_ISR=0, OTP_IN_ISR=0; //banderas de isr 
volatile int  TEMP_1=0, TEMP_2=0, TEMP_3=0, CLEAR_OTP=0; 
volatile int se_ack, ocp_ack, otp_ack; // ack para que ?
volatile int32 Modo = NORMAL; //modo funcionamiento 
//
//
void main(void)
{
    ...
    //
    InitSysCtrl(); // initializing the system control
    EALLOW; // This line of code disables the EAL (Extended Absolute Low-Power)
    ClkCfgRegs.PERCLKDIVSEL.bit.EPWMCLKDIV = 1; // 
    EDIS; //This line of code disables the global interrupt enable (GIE) bit in the EDIS register. 
    //
    ...
    //
    EALLOW;                                                     // Pide permiso para modificar registro EN QUE QUEDAMO 
    ...
    // Configura el ePWM
    //
    initEpwm();                                                 // Esta funcion est� en Example_EPwmSetup.c
    InitEPwm2(Period, phaseOffset2, Duty2); //CONTROL INIT PWM 
    ...
    for(;;){
        ...
        if (doSample){
            Muestreo();
            doSample = 0; // EN 1 CUANDO SE REALIZO CONVERSION DE ADC
            ...
            if (ComprobarLimites() || SE_SIGN ) // se_sign es valor en tiempo real de la parada de emergencia
            {                                                        //  --Frenan todos los motores -- La se�al ePWM entra al pin DIR de cada driver
                    GpioDataRegs.GPBCLEAR.bit.GPIO61 = 1;           // Baja el pin PWM GPIO61 motor 1 
                    GpioDataRegs.GPDCLEAR.bit.GPIO123 = 1;          // Baja el pin PWM GPIO123 motor 2 
                    GpioDataRegs.GPDCLEAR.bit.GPIO122 = 1;          // Baja el pin PWM GPIO122 motor 3 
        ...
        // interrupcion para loop de control, cuando hay una interrupcion del adc 
        if (doLoop){
                switch(Modo){
                case NORMAL:
                    Control();
...
interrupt void adca1_isr(void){
    ...
    AdcaRegs.ADCINTFLGCLR.bit.ADCINT1 = 1;          // Borra la INT1 flag ADC
