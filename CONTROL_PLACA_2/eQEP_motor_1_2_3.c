...
void POSSPEED_1_Calc(POSSPEED *p)
{
        ...
        if((p->theta_raw < 4294967296)&&(p->theta_raw > 4.29486694e9/2)){
           ...
           p->angle_robot = Naxis4*p->angle_robot;
        }
...
void POSSPEED_2_Calc(POSSPEED *p)
{
    ...
    if((p->theta_raw < 4294967295)&&(p->theta_raw > 4294865105/2)){
       ...
       p->angle_robot = Naxis5*p->angle_robot;
    }
...
void POSSPEED_3_Calc(POSSPEED *p)
{
    ...
    p->theta_raw = pos32bval+ p->cal_angle; // CAL ANGLE OFFSET

    if((p->theta_raw < 4294967295)&&(p->theta_raw > 4294865105/2)){
       p->angle_robot = -(1/2048)*(p->theta_raw); //ACOMODAR Y VERIFICAR ESTO
       p->angle_robot = Naxis6*p->angle_robot;
    }
    if((p->theta_raw < 2*102540)&&(p->theta_raw > 0)){
       p->angle_robot = -(1/2048)*(p->theta_raw);
       p->angle_robot = Naxis5*p->angle_robot;
    }
    if(p->theta_raw == 0){
        p->angle_robot=0;
